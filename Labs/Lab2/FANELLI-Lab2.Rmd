...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    __ANSWER:__ From above: "we define the length of a shooting streak to be the number of consecutive baskets made until a miss occurs", so a streak of 1 implies 1 hit - not preceded by a hit - followed by a miss.  Saying "a streak of length zero" means 2 misses in a row.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    __ANSWER:__ Kobe's shot distribution looks like the right side of a normal distribution.  There are surprisingly few "streaks"" of 2 or more baskets. The typical streak length is 0, followed by 1, then 2 and 3, and then 4.  His longest streak was 4.
...
3.  In your simulation of flipping the unfair coin 100 times, how many flips 
    ...
    __ANSWER:__ The first time run, I got exactly 20 heads and 80 tails.  This seemed a little too perfect, so upon a second run, it came back 27 heads and 73 tails.
...
4.  What change needs to be made to the `sample` function so that it reflects a 
    ...
    __ANSWER:__ Add the "prob" parameter to the sample function call, and update the sample size to equal Kobe's number of shots: 
