...
__This relationship looks somewhat linear to me, slightly rising.__
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    __The relationship is slightly positive, but very spread out.  The form is very spread, the direction is only * slightly * positivem and the relation does not look all that strong.__
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    * run 1: Sum of Squares:  142945.6
    * run 2: Sum of Squares:  254186.1
    * run 3: Sum of Squares:  278651.5
    * [All greater than the least squares value of 123721.9]
...
__regression line equation:__

\[
  \hat{y} = 415.2389 + 1.8345 * homeruns
\]

The slope using the home runs variable being greater than the atbats variable tells us that increase in home runs correlates more directly to increase in wins than do at_bats. 
...
```{r}
# http://stackoverflow.com/questions/24882209/how-to-get-trendline-equations-in-r
coef(m1)
paste('y =', coef(m1)[[2]], '* x', '+', coef(m1)[[1]])

actual_at_bats <- 5578
predicted_hr <- coef(m1)[[2]] * actual_at_bats + coef(m1)[[1]]
predicted_hr
```

This seems like an overestimate, because there are 2 points well above the line, but a whole bunch slightly below it.

Residuals for this prediction:
...
__This seems to me to show that there is no relationship between runs and at bats.__
...
___The nearly normal residuals condition DOES appear to be met___
...
___Yes, (1) seems to be consistently variable.    
