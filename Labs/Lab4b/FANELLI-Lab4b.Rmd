...
"typical" size within your sample? Also state precisely what you interpreted "typical" to mean.

It is skewed right, with a typical size of about 20. I interpreted "typical" to mean average, median...mean...the middle...
...
I would not expect another student's distribution to be identical to min,  It might be similar though since these are somewhat normal distributions.
...
__Answer:__ There must be a sufficient number of samples with no extreme outliers.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

__Answer:__ A 95% confidence interval has a 0.95 probability of containing the mean
...
__Answer:__ The confidence interval does capture the true mean, since the confidence interval is based on the mean.
...
__Answer:__ I would expect 95% to capture the true population mean because we built it based on being within 2 SD's
